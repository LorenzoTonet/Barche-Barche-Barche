...
class ActorCriticNetwork(torch.nn.Module):
    def __init__(self, input_dim, hidden_dimension = 128, action_dim = 1):
        ...
        self.actor_sigma = torch.nn.Sequential(
            torch.nn.Linear(hidden_dimension, action_dim),
            torch.nn.Softplus()
        )
    ...
    def forward(self, x):
        ...
        action_mu = self.actor_mu(action_features)
        action_sigma = self.actor_sigma(action_features)
...
class SharedActorCriticNetwork(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        action_mu = self.actor_mu(shared_output)
        action_sigma = self.actor_sigma(shared_output)
...
class PPOAgent():
    ...
    def update(self):
        ...
        for _ in range(self.epochs):
            ...
            mu, sigma, state_values = self.network(states)
